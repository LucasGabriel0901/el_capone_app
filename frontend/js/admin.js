const API_URL = "https://el-capone-app.onrender.com/api";
...
async function cadastrarColaborador() {
    const campoWhatsApp =
        ...
        document.getElementById('adminWhatsapp') ||
        document.getElementById('adminTelefone');
    ...
    try {
        ...
        try {
            ...
            alert(`Erro no servidor. Status: ${resposta.status}`);
            return;
        }

        if (!resposta.ok) {
            alert(dados.mensagem || `Erro HTTP ${resposta.status}`);
            return;
        }

        alert(dados.mensagem || "Colaborador cadastrado com sucesso.");
        alternarTelaAdmin('login');
    } catch (erro) {
    ...
    try {
        ...
        if (pendentes.length === 0) {
            ...
            pendentes.forEach(agenda => {
                ...
                linha.innerHTML = `
                    <td>
                        <b>${agenda.cliente_nome}</b><br>
                        <small style="color: #888;">${agenda.cliente_telefone}</small>
                    </td>
                    ...
                    <td>
                        <span style="color: #d4af37; font-weight: bold;">
                            ⏳ ${traduzirStatusAgendamento(agenda.status)}
                        </span>
                    </td>
                    <td style="display: flex; gap: 10px; align-items: center; border: none;">
                        <button class="btn-detalhes" onclick="abrirModalEdicao(${agenda.id}, '${agenda.data}', '${agenda.hora}')">
                            EDITAR
                        </button>
                        <button class="btn-dourado-full" style="padding: 5px 10px; font-size: 11px; width: auto; margin: 0;" onclick="concluirAgendamento(${agenda.id})">
                            CONCLUIR
                        </button>
                    </td>
                `;
        ...
        if (tabelaHistorico) {
            if (historico.length === 0) {
                ...
                historico.forEach(agenda => {
                    const corStatus = agenda.status === 'concluido' ? '#25d366' : '#ff4444';
                    const icone = agenda.status === 'concluido' ? '✅' : '❌';
                    ...
                    linha.innerHTML = `
                        ...
                        <td>
                            <span style="color: ${corStatus}; font-weight: bold;">
                                ${icone} ${traduzirStatusAgendamento(agenda.status)}
                            </span>
                        </td>
                    `;
...
async function concluirAgendamento(id) {
    if (confirm("Deseja marcar este serviço como concluído?")) {
        try {
            const resposta = await fetch(`${API_URL}/agendamentos/${id}/concluir`, {
                method: 'PUT'
            });
            ...
            if (resposta.ok) {
                ...
                if (typeof carregarAnalise === 'function') {
                    carregarAnalise();
                }
            }
        } catch (erro) {
        ...
        clientes.forEach(c => {
            div.innerHTML += `
                <div style="background:#111; padding:15px; margin-bottom:10px; border-left:3px solid #d4af37; border-radius: 4px;">
                    <h3 style="color: #d4af37; margin: 0 0 8px 0; font-family: 'Oswald', sans-serif;">
                        ${c.nome.toUpperCase()}
                    </h3>
    ...
    try {
        ...
        horarios.forEach(slot => {
            ...
            if (slot.estado === 'Ocupado') {
                ...
                btn.style.backgroundColor = "#154a1f";
...
async function adicionarHorarioExtra() {
    ...
    try {
        ...
        document.getElementById('novoHorarioExtra').value = "";
        carregarDisponibilidadeDoDia();
    } catch (erro) {
        ...
        grid.innerHTML = `
            ...
            <div style="background:#151515; border:1px solid #25d366; padding:20px; border-radius:4px; text-align:center;">
                ...
                <p style="color:#fff; font-size:24px; font-weight:bold;">
                    ${metricas.concluidos}
                    <small style="font-size:12px;">(${metricas.pct_concluido}%)</small>
                </p>
            </div>
            ...
            <div style="background:#151515; border:1px solid #ff4444; padding:20px; border-radius:4px; text-align:center;">
                ...
                <p style="color:#fff; font-size:24px; font-weight:bold;">
                    ${metricas.cancelados}
                    <small style="font-size:12px;">(${metricas.pct_cancelado}%)</small>
                </p>
            </div>
            ...
            <div style="background:#151515; border:1px solid #333; padding:20px; border-radius:4px; text-align:center;">
                ...
                <p style="color:#fff; font-size:24px; font-weight:bold;">
                    R$ ${Number(metricas.valor_previsto).toFixed(2)}
                </p>
            </div>
            ...
            <div style="background:#151515; border:1px solid #25d366; padding:20px; border-radius:4px; text-align:center;">
                ...
                <p style="color:#fff; font-size:24px; font-weight:bold;">
                    R$ ${Number(metricas.valor_concluido).toFixed(2)}
                </p>
            </div>
            ...
            <div style="background:#151515; border:1px solid #ff4444; padding:20px; border-radius:4px; text-align:center;">
                ...
                <p style="color:#fff; font-size:24px; font-weight:bold;">
                    R$ ${Number(metricas.valor_perdido).toFixed(2)}
                </p>
            </div>
        `;
    } catch (erro) {
